...
/** 各维度在综合分中的权重 */
export const WEIGHT_SCORE = 0.4
export const WEIGHT_TIME = 0.2
export const WEIGHT_PROPS = 0.25
export const WEIGHT_CLICK = 0.15
...
export function calcStars(input: StarInput): number {
  ...
  const fScore = Math.min(1, score / (tileCount * SCORE_PER_TILE))
  const fTime = Math.min(1, timeLeft / timeLimit)
  const fProps = Math.max(0, 1 - propsUsed / PROPS_SATURATION)
