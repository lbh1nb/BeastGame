...
**已确认（2026-08-04）**：当前 code 模式的 Seedance 工具**不可用**，无法生成视频/图片；只有 **work 模式**可用 Seedance / Seedream。

协作分工：
- **work 模式 agent**：负责生成图片与动画素材，放入 `resources/` 对应目录。
- **code 模式 agent（本 Agent）**：负责继续开发项目并集成素材。

注意事项：
- 不要强行调用 Seedance / `GenerateVideo`（会失败）。
- 需要图片/视频/动画素材时，请 work 模式 agent 生成后放到资源目录，再由 code 模式集成。
- 涉及视觉/动画需求时，优先把这些需求交给 work 模式 agent 出素材。
