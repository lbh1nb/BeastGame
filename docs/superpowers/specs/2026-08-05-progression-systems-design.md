...
| 维度 | 说明 | 权重 |
|------|------|------|
| 本局分数 Score | 游戏内累计分数 | 40% |
| 剩余时间 Time | 通关剩余时间（Boss 关用），单位秒 | 20% |
| 道具使用 Props | 本局使用的撤回/洗牌/提示/新道具次数，越少越好 | 25% |
| 剩余点击 Click | 藤蔓/气泡关剩余点击数，越多越好 | 15% |

- 综合分 `starScore = 0.4*fScore + 0.2*fTime + 0.25*fProps + 0.15*fClick`，∈ [0,1]
